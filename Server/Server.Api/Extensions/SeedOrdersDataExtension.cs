...
internal static class SeedOrdersDataExtension
{
    public static async Task SeedOrdersDataAsync(this IApplicationBuilder app)
    {
        ...
        const int totalOrders = 200; // Reduced for initial testing
        ...
        try
        {
            ...
            if (targetStatus != OrderStatus.Pending && targetStatus != OrderStatus.Cancelled)
            {
                ...
                if (processResult.IsSuccess)
                {
                    ...
                    order.AttachPaymentToOrder(payment.Id, payment.Amount);
    ...
    private static OrderInfo GenerateOrderInfo(int orderIndex)
    {
        string info = $"Order #{orderIndex} - Created on {DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}";

        var orderInfo = OrderInfo.Create(info);
