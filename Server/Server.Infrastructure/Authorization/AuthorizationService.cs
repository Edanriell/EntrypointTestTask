...
internal sealed class AuthorizationService
{
    ...
    public async Task<UserRolesResponse> GetRolesForUserAsync(string identityId)
    {
        ...
        UserRolesResponse? cachedRoles = await _cacheService.GetAsync<UserRolesResponse>(
            cacheKey
        );
        ...
        UserRolesResponse roles = await _dbContext.Set<User>().Where(u => u.IdentityId == identityId
    ...
    public async Task<HashSet<string>> GetPermissionsForUserAsync(string identityId)
    {
        ...
        ICollection<Permission> permissions = await _dbContext.Set<User>().Where(u => u.IdentityId == identityId
        ).SelectMany(u => u.Roles.Select(r => r.Permissions
            )
        ).FirstAsync();

        var permissionsSet = permissions.Select(p => p.Name
        ).ToHashSet();
