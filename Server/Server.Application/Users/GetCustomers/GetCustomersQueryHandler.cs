...
//
//
//         IEnumerable<Customer> customers = await connection.QueryAsync<Customer>(customerSql);
//         IEnumerable<RecentOrder> recentOrders = await connection.QueryAsync<RecentOrder>(recentOrdersSql);
//
//         var recentOrdersByCustomer = recentOrders
//             .GroupBy(o => o.UserId)
//             .ToDictionary(g => g.Key, g => g.Take(5).ToList());
//
//         var customersWithOrders = customers.Select(customer =>
//         {
//             customer.RecentOrders = recentOrdersByCustomer.GetValueOrDefault(customer.Id, new List<RecentOrder>());
//             return customer;
//         }).ToList();
//
//         return new GetCustomersResponse(customersWithOrders);
//     }
// }
...
//
//         // Add LIMIT
//         parameters.Add("PageSize", request.PageSize + 1); // +1 to check if there's a next page
...
//         if (hasNextPage && customers.Count > 0)
//         {
//             Customer lastCustomer = customers[^1];
//             nextCursor = GenerateCursor(lastCustomer, request.SortBy);
//         }
//
//         // Get recent orders for each customer
//         await LoadRecentOrders(connection, customers);
//
//         // Get total count
...
//         {
//             string cursorValue = DecodeCursor(request.Cursor);
...
//             parameters.Add("CursorValue", cursorValue);
//         }
//     }
//
...
//     }
//
//     private string GenerateCursor(Customer customer, string? sortBy)
//     {
//         string cursorValue = sortBy?.ToLower() switch
//         {
...
//                                        FROM orders o
//                                        LEFT JOIN order_products op ON o.id = op.order_id
//                                        WHERE o.user_id = ANY(@CustomerIds)
//                                        AND o.created_at >= CURRENT_DATE - INTERVAL '30 days'
//                                        GROUP BY o.user_id, o.id, o.status, o.created_at
//                                        ORDER BY o.created_at DESC
//                                        """;
