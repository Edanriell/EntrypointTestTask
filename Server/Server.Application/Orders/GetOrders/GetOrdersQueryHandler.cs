...
//
//         // Base query using the actual database schema from OrderConfiguration
//         sqlBuilder.Append("""
//                           SELECT 
//                             -- Order columns
//                             o.id AS Id,
...
//                             latest_payment.amount AS PaymentTotalAmount,
//                             latest_payment.payment_status AS PaymentStatus,
//                               
//                             -- Order Product data (✅ REMOVED the ID field as requested)
...
//                           LEFT JOIN LATERAL (
//                               SELECT p1.id, p1.amount, p1.payment_status, p1.created_at
//                               FROM payments p1
...
//         var orderDictionary = new Dictionary<Guid, OrdersResponse>();
//
//         await connection.QueryAsync<OrdersResponse, ClientResponse, PaymentResponse, OrderProductResponse
//             , OrdersResponse>(
//             sqlBuilder.ToString(),
...
//
//                     OrdersResponse updatedOrder = existingOrder with { OrderProducts = updatedProducts };
...
//         //     order => GetOrderSortValue(order, request.SortBy));
//
//         PaginationInfo<OrdersResponse> paginationInfo = _cursorPaginationService.DeterminePaginationState(
//             orders,
//             request.PageSize,
...
//             order => GetOrderSortValue(order, request.SortBy ?? "CreatedOnUtc"));
//
//         // Get total count
...
//     }
//
//     private void AddSorting(StringBuilder sqlBuilder, GetOrdersQuery request)
//     {
...
//             "clientemail" => "u.email",
//             "paymentstatus" => "latest_payment.payment_status",
//             "paymentamount" => "latest_payment.amount",
//             "courier" => "o.courier",
...
//             "paymentamount" => order.Payment?.PaymentTotalAmount ?? 0,
...
//         if (!string.IsNullOrEmpty(request.PaymentStatusFilter))
//         {
//             sqlBuilder.Append(" AND latest_payment.payment_status ILIKE @PaymentStatusFilter");
//             parameters.Add("PaymentStatusFilter", $"%{request.PaymentStatusFilter}%");
//         }
//
//         // New filters
...
//             {
//                 sqlBuilder.Append(" AND latest_payment.id IS NOT NULL");
//             }
//             else
//             {
//                 sqlBuilder.Append(" AND latest_payment.id IS NULL");
//             }
//         }
//
...
//                           """);
//
//         // Add filtering to the CTE
//         AddFiltersToBaseQuery(sqlBuilder, parameters, request);
//
//         // Add sorting to the CTE
//         AddSortingForCTE(sqlBuilder, request);
//
//         // Add pagination to the CTE - this limits ORDERS, not joined rows
//         parameters.Add("PageSize", request.PageSize + 1); // +1 to check for next page
//         parameters.Add("Offset", offset);
//         sqlBuilder.Append(" LIMIT @PageSize OFFSET @Offset");
//
//         // Close the CTE and select from it with all the joins
//         sqlBuilder.Append("""
//                           )
//                           SELECT 
...
//                           """);
//
//         // Add the same sorting to the final result to maintain order (using po. prefix)
...
//     private void AddFilters(StringBuilder sqlBuilder, DynamicParameters parameters, GetOrdersQuery request)
//     {
//         // This method is kept for backward compatibility and total count calculation
//         AddFiltersToBaseQuery(sqlBuilder, parameters, request);
//     }
//
...
//     {
//         // Legacy method - kept for compatibility
//         return GetSortColumnForCTE(sortBy);
//     }
//
//     private async Task<int> GetTotalCount(IDbConnection connection, GetOrdersQuery request)
//     {
        ...
        await connection
            .QueryAsync<OrdersResponse, ClientResponse, PaymentResponse, OrderProductResponse, OrdersResponse>(
                ...
                (order, client, payment, orderProduct) =>
                {
                    if (!orderDictionary.TryGetValue(order.Id, out OrdersResponse? existingOrder))
                    {
                        // Create payments list - handle null case properly
